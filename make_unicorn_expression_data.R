...
diet_effect <- replicate(6, {
  runif(1, 100, 105) + ifelse(data$diet == "flowers", 1, 0) + rnorm(nrow(data))
})

colour_effect <- replicate(3, {
  runif(1, 100, 105) + ifelse(data$colour == "pink", 2, 0) + rnorm(nrow(data))
})

no_effect <- replicate(3, {
  runif(1, 100, 110) + rnorm(nrow(data))
})
